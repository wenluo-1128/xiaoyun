...
logger = logging.getLogger('ing_forms_query_client')

# 后端API基础URL
BASE_URL = "http://localhost:3005"

def save_json_response_to_file(json_data) -> bool:
    """将API返回的JSON数据保存到3.json文件中
    ...
    """
    json_file_path = "c:/play_vue3/hd/test/3.json"
...
def query_ing_forms(
    ...
    """查询ing-forms数据，调用/api/ing-forms/query接口
    ...
    try:
        logger.info(f"准备查询ing-forms数据: {url}, 参数: {query_params}")
        print(f"准备查询ing-forms数据: {url}")
...
def get_query_params():
    """通过命令行输入获取查询参数，只需要userid参数
    
    Returns:
        dict: 包含user_id的查询参数字典
    """
    while True:
        print("请输入查询参数:")
        user_id = input("用户ID (user_id): ").strip()
        if user_id:
            return {'user_id': user_id}
        print("错误: user_id不能为空，请重新输入")
...
def display_menu():
    ...
    print("           ing-forms查询工具            ")
    print("="*50)
    print("1. 调用 /api/ing-forms/query 接口进行查询")
    print("0. 退出")
    print("="*50)

def main():
    """主函数"""
    print("ing-forms查询工具已启动")
    ...
    while True:
        ...
        if choice == '0':
            ...
            print("\n您选择了: 调用 /api/ing-forms/query 接口进行查询")
            
            # 获取查询参数
            query_params = get_query_params()
            
            # 调用查询接口
            result = query_ing_forms(query_params=query_params)
